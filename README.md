...
| `host_permissions: https://www.linkedin.com/*` | Inject the content script and query LinkedIn tabs to set the badge |
...
We need to:
1. Inject a content script on LinkedIn — covered by `content_scripts.matches`
2. Send messages from popup to LinkedIn tabs via `chrome.tabs.sendMessage`
3. Query LinkedIn tabs from the background to set per-tab badges via `chrome.tabs.query({ url })`

The `tabs` permission is misleadingly named — it doesn't gate access to `chrome.tabs.*`. It only gates reading four sensitive `Tab` properties (`url`, `pendingUrl`, `title`, `favIconUrl`). On the install screen it surfaces as "Read your browsing history" which scares users.
